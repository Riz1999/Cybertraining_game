...
export const register = async (userData) => {
  try {
    ...
    const directUrl = `${API_URL}/auth/register`;
...
export const login = async (credentials) => {
  try {
    ...
    // Try using fetch API instead of axios
    const directUrl = `${API_URL}/auth/login`;
    console.log('Attempting fetch to:', directUrl);
...
export const testApiConnection = async () => {
  try {
    ...
    try {
      const fetchResponse = await fetch(`${API_URL}/test`);
      if (fetchResponse.ok) {
    ...
    try {
      const axiosResponse = await axios.get(`${API_URL}/test`);
